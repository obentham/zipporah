...
if [ "$aligner" == "fast-align" ]; then
  ...
  if [ "$align_job" == "" ] || [ "$align_job" == "1" ]; then
    ...
    $fast_align/build/fast_align -i $tmpfolder/pasted -d -o -v > $alignoutput.forward
    $fast_align/build/fast_align -i $tmpfolder/pasted -d -o -v -r > $alignoutput.backward
    $fast_align/build/atools -i $alignoutput.forward -j $alignoutput.backward -c grow-diag-final-and > $alignoutput
  else
